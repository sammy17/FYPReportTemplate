...
\par We would like to convey our deepest gratitude to Xilinx, Inc for providing us with Xilinx Zynq-7000 All Programmable SoC ZC702 Evaluation Kits.\\
...
A central server receives the vectorized information to be used for multicamera people tracking. There are several ZC702 devices interfaced with a camera which will be sending the people detection results to the server through Ethernet. Server fuses all these results and performs people tracking in a multicamera system. 

\item  Business Intelligence Software \\
End result of the project is to assist making business decisions in a large retail store like environment. The end system uses the multi-camera people tracking results to visualize the paths of the people using heat maps of people density and heat graphs, etc.
